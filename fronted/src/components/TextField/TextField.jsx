      ...
      <div ref={ref} className="inline-flex flex-col">
         ...
         ${isClicked ? "outline-2 outline-primary"  : "outline-1 outline-outline hover:outline-onSurface"}`}>
            ...
            <input 
               ...
               onFocus={() => {if (notMenu) {
                  setIsClicked(true)} else {
                     handleClick();
                     onClear();
                  }
               }}
               onBlur={() => {if (notMenu) {
                  setIsClicked(false)} else setHasText(false) 
               }}
            ...
            {(type === TextFieldTypes.Menu && !isClicked) && <div className="bg-transparent flex justify-center items-center w-6 h-6 mr-3">
